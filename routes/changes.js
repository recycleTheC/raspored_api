...
router.post(
  ...
  [
    ...
    [
      ...
      body("changed").not().isEmpty(),
      body("substitution").not().isEmpty(),
...
router.put("/:id", auth, async (req, res) => {
  const { date, classId, changedKey, substitutionKey } = req.body;

  // Build contact object
  const fields = {};
  if (date) fields.date = date;
  if (classId) fields.classId = classId;
  if (changedKey) fields.changedKey = changedKey;
  if (substitutionKey) fields.substitutionKey = substitutionKey;
